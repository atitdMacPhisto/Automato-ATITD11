...
-- v1.0.3 by Kavad
-- Made it more flexible in detecting the treatment bars, regardless of extra lines in the Treatment Tank header
--
...
function calcPos()
  ...
  treat = findText("Treat...", tank);
    ...
    if treat then
      ...
      posFlexibility = {statsX, statsY};
      posCuttability = {statsX, statsY + 16};
      posFlammability = {statsX, statsY + 32};
      posWaterResist = {statsX, statsY + 48};
      posInsectTox = {statsX, statsY + 64};
      posHumanTox = {statsX, statsY + 80};
      posDarkness  = {statsX, statsY + 96};
      posGlossiness = {statsX, statsY + 112};
    end
end
...
function doit()
  ...
  askForWindow(
    "Wood Treatment Tank - K Values v1.0.3\n\n" ..
    "Pin your Wood Treatment Tank window. Then press shift.\n\n"
  );
  calcPos();
  ...
  while not finished do
    ...
    while not done do
      lsPrintWrapped(10, 10, 5, lsScreenX-15, 0.6, 0.6, 0xFF0000ff,
        "Load the boards and refresh the tank window BEFORE treating,"
        .. " otherwise the window movement will break the read position!\n\n"
        .. "(Read the Wood Treatment Guide on wiki, the part about the K-Values"
        .. ", if you don't know what to do).\n\n");
      lsPrintWrapped(10, 105, 5, lsScreenX-15, 0.7, 0.7, 0xFFFFFFFF,
        "" .. suggest .. suggestion);
