...
function runKettles(num_loops, action)
  ...
  for i=1, num_loops do
    ...
    clickAllImages("kettle/begin.png");
    lsSleep(200);
...
function igniteAll()
  srReadScreen();
  local ignite = findAllImages("kettle/ignite.png");
  for i=1,#ignite do
    srClickMouseNoMove(ignite[i][0] + 5, ignite[i][1] + 5);
    local maxButton = waitForImage("max.png", 500,
...
function stokeWindow(anchor, stoked)
  ...
  local takePos = findImageInWindow("kettle/take.png", anchor[0], anchor[1], bounds);
  if not takePos then
    done = false;
    if stoked then
      local wood;
      ...
      then
        local stoke = findImageInWindow("kettle/stoke.png", anchor[0], anchor[1], bounds);
        if stoke then
